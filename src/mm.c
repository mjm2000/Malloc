...
void *realloc(void *ptr, size_t size) {
 //  fprintf(stderr,"RE");   
    ...
    size_t og_value = *(size_t*)(ptr - sizeof(size_t));
    ...
    if ( size <= og_value ){
        ...
        if (  size  <= CHUNK_SIZE  ){
            ...
            memcpy(new_node, ptr-1-sizeof(size_t) ,og_value -1 );
 ...
 //   heap_validator();
  ...
  //  fprintf(stderr,"FREE%p--------------------------------- \n",(void*)size_block );
     ...
     //   fprintf(stderr,"%zu header\n", *size_block ) ;
      ...
      //  fprintf(stderr ,"%zu block_val\n", block_val ) ;
        // if it was bulk allocated it will be bulk freed 
        if(*(size_t*)size_block-1 > CHUNK_SIZE){
